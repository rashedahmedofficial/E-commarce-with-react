...
const Productdetails = () => {
  ...
  const triggerPopup = (message, isError = false) => {
  ...
  const handleAddToCart = () => {
    try {
      ...
      if (found) {
        triggerPopup("Already in cart!", true);
        return;
      } else {
        updated = [
          ...existing,
        ...
        triggerPopup("Added to Cart Successfully!"); 
      }
  ...
  return (
    <section className="py-14 relative">
      ...
      {popup.show && (
        <div className={`fixed top-5 right-5 z-50 flex items-center gap-3 px-6 py-4 rounded-lg shadow-xl text-white transition-all duration-300 transform translate-y-0 ${
          popup.isError ? "bg-red-500" : "bg-green-500"
        }`}>
          {!popup.isError && <FaCheckCircle className="text-xl" />}
      ...
      <div className="container grid grid-cols-2 gap-28">
        ...
        <div>
          ...
          <div className="flex gap-6 items-center">
            ...
            {size.map((item) => (
              <label
                key={item}
                htmlFor={item}
                className={`py-1 px-3 border border-secondary/10 cursor-pointer ${
                  selectedSize === item && "bg-brand text-white"
                }`}
              >
                {item}
                <input
          ...
          <div className="py-4 flex gap-8 items-center mt-4 ">
            ...
            <button
              className="bg-brand text-white px-4 md:px-6 py-2 rounded cursor-pointer"
              onClick={handleAddToCart}
            >
              Add to cart
            </button>
            <Link to="/Login" className="border-2 border-blue-400 px-6 py-1.5 bg-blue-50 font-semibold text-blue-400 rounded">
              Buy Now
            </Link>
          </div>
        </div>
